/*http보다 개선된 express모듈 이용한 서버구축
express 모듈은 미들웨어가 많이 지원된다 
Node.js 에서의 미들웨어는 함수로 정의되어 있다.
...
//정적자원은 라우팅의 대상이 아니다. 따라서 정적자원위 위치를 지정하면 
//별도의 라우팅이 필요없다 (스프링의 경우엔 요청을 처리할 컨트롤러 지정이 필요없다)
//__dirname: node.js 의 전역변수 중 하나 (?웹 서버의 루투 경로를 반환 )
...
//게시판 목록 요청   
//클라이언트의 기종에 종속되면 안된다 서버는 다양한 디바이스에 대응해야한다 
//따라서 데이터 전송은 되도록이면 중립적 데이터로 보내야한다!! (XML, JSON)
/*
...
app.get("/notice/list", function(request, response){
    ...
    con.query(sql, function(error,result,fields){
        if(error){
            ...
            //클라이언트의 종류를 알 수 엊ㅅ으므로, 중립적 형태의 데이터인 json으로 응답하자
            console.log(result);
            var record = {
                list:result
            }
